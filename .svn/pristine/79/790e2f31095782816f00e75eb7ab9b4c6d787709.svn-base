...
function containsProp(array, day ,obj) {
  if (array) {
    var i = array.length;
    while (i--) {
     var objarray=array[i].date.split("-") ;
      if (Number(objarray[2]) == day){
        ...
        var caloris = (weight * steps / 1000 * stepWidth * k).toFixed(1);
        var kms = (stepWidth * steps / 1000).toFixed(1); //公里
        
        obj.tmpSteps = steps;
        obj.tmpKms = kms;
        obj.tmpCaloris = caloris;

        obj.steps = (steps / 10000).toFixed(1) + "万";
        if (steps >= 10000){
          return true;
        }else{
          return false;
